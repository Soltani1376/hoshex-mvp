...
const cloud = fs.readFileSync(path.join(root, "assets", "hoshex-cloud.js"), "utf8");
...
for (const [name, source] of [["cloud", cloud], ["widget", widget]]) {
...
check(cloud.includes("/rest/v1/"), "client sync uses Supabase Data API");
...
check(widget.includes("journeys?user_id=eq."), "website widget reads same cloud journey");
check(plugin.includes("hoshex_business_path"), "WordPress shortcode is registered");
check(plugin.includes("hoshex-cloud-widget.js"), "WordPress plugin loads shared cloud widget");
...
check(analytics.includes('/assets/hoshex-cloud.js'), "app loader includes cloud JS");
